...
class Newtable {
	...
	function generate($table_data = NULL)
	{
		...
		{
			//print_r($this->keycari);
			$out .= '<span>Cari&nbsp;&nbsp;<select id="tb_keycari'.$this->formid.'" title="Pilih kategori yang akan dicari" '.$disabled.' onChange="TagChange(this.id)" style="height:23px;margin:0px;min-width:3px;padding:2px">';
			...
			foreach ($this->keycari as $a => $b){
				foreach ($b as $c => $d){
					if($d=="tag-select"){
						...
						$tagtanggal .= '<input type="text" id="tb_cari'.$this->formid.'_tgl1" title="Masukkan tanggal" onfocus="ShowDP(this.id);" onmouseover="ShowDP(this.id);" value="'.$tanggal1.'" style="height:22px;margin:0px;width:90px"/>';
						$tagtanggal .= '&nbsp;s/d&nbsp;<input type="text" class="text date" id="tb_cari'.$this->formid.'_tgl2" title="Masukkan tanggal" onfocus="ShowDP(this.id);" onmouseover="ShowDP(this.id);" value="'.$tanggal2.'" style="height:22px;margin:0px;width:90px"/>';
					}
			...
			$out .="&nbsp;<button type=\"button\" class=\"btn btn-primary btn-xs\" OnClick=\"tb_cari('".$this->actions."', '".$this->divid."', '".$this->baris."','".$this->orderby."', '".$this->sortby."','tb_hal".$this->formid."','tb_cari".$this->formid."','tb_keycari".$this->formid."');\"><i class=\"fa fa-search\"></i>Cari</button>";
			...
			$out .= "<script> 
						...
						$('#TagChange".$this->formid."').html('".$tagtanggal."');
						}else{ $('#TagChange".$this->formid."').html('".$taginput."'); }
		...
		if (count($this->rows) > 0){
			if (count($this->heading) > 0)
			{
				...
				if ( $this->detils_tipe!="" and $this->detils_tipe != "detil_priview_blank" ){
		...
		if (count($this->rows) > 0)
		{
			...
			foreach($this->rows as $row)
			{
				...
				if($this->detils_tipe=="pilih"){ 
					$out .= $cels."<button type=\"button\" name=\"pilih".$this->formid."\" id=\"pilih".$this->formid."\" class=\"btn btn-mini btn-default\" onclick=\"td_pilih('".$this->formField."|".$keypilih."|".$field."');\"><i></i>Pilih</button></td>";
				}
				...
				elseif($this->detils_tipe=="pilih_proses"){ 
					$out .= $cels."<button type=\"button\" name=\"pilih".$this->formid."\" id=\"pilih".$this->formid."\" class=\"btn btn-mini btn-default\" onclick=\"".$this->detail_proses."('".$keypilih."');\"><i></i>Pilih</button></td>";
				}
		...
		{ echo "";
			...
			if($total_record > $this->baris){ 
				...
				$out .="Halaman <input type=\"text\" class=\"tb_text\" id=\"tb_hal".$this->formid."\" title=\"Masukkan nomor halaman yang diinginkan kemudian klik Go\" value=\"".$this->hal."\" ".$disabled."  ondblclick=\"".$nextExec."\" style=\"width:30px;text-align:right;\"/>"; 
				
				$out .="&nbsp;<input type=\"button\" class=\"btn btn-primary btn-mini\" OnClick=\"tb_go('".$this->actions."', '".$this->divid."', '".$this->baris."','".$this->orderby."', '".$this->sortby."','tb_hal".$this->formid."', 'tb_keycari".$this->formid."', 'tb_cari".$this->formid."');\" value=\"&nbsp;Go&nbsp;\">";
	...
	function _set_from_object($query)
	{
		...
		if (count($this->heading) == 0)
		{
			...
			if( $this->show_chk ){
				if($this->check != "radio"){
					$this->heading[] .= "<div><input type=\"checkbox\" id=\"tb_chkall".$this->formid."\" onclick=\"tb_chkall('".$this->formid."')\" class=\"tb_chkall\"/></div>";
				}else{
					$this->heading[] .= "&nbsp;";	
				}
			}
