...
class CI_Table {
	 ...
	 * @return	string
	 */
	function generate($table_data = NULL, $order = "", $db = "", $hal = 1, $baris = 20)
	{
		...
		if ( ! is_null($table_data))
		{
			...
			elseif ($table_data!="")
			{
				...
				if ($hal==1)
				{
					$table_data = "SELECT TOP $baris ".substr($table_data, 6);
					$table_data = $db->query($table_data);
				}
				else
				{
					$dari = ($hal * $baris) - $baris + 1;
					$sampai = $hal * $baris;
					
					$table_data = "SELECT * FROM (SELECT ROW_NUMBER() OVER (ORDER BY $order) AS HAL, ".substr($table_data, 6).") AS TBLTMP WHERE HAL >= $dari AND HAL <= $sampai";
					//echo $table_data; die();
					$table_data = $db->query($table_data);
				}
				$this->_set_from_object($table_data);
	...
	function _set_from_object($query)
	{
		...
		if ($query->num_rows() > 0)
		{
			...
			{
				if (array_key_exists('HAL', $row)) unset($row['HAL']);
				$this->rows[] = $row;
			}
		}
	}
